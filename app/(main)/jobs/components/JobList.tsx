...
export default function JobList({ jobs, savedJobs, viewMode, filters, onFilterChange }: JobListProps) {
    ...
    const handleWorkingTypeClick = (workingType: string) => {
        if (onFilterChange && filters) {
            onFilterChange({
                ...filters,
                workingType: filters.workingType && filters.workingType.includes(workingType) ? [] : [workingType],
            });
            window.scrollTo({ top: 0, behavior: 'smooth' });
        }
    };
